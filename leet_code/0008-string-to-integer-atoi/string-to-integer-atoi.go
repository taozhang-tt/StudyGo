...
func myAtoi(str string) int {
	ret, start, sign := 0, 0, 1
	for start = 0; start < len(str); start++ { //去除开头空格
		if str[start] != ' ' {
			break
		}
	}
...
func myAtoi2(str string) int {
	stateTable := map[string][]string{
		"start":  []string{"start", "sign", "number", "end"},
		"sign":   []string{"end", "end", "number", "end"},
		"number": []string{"end", "end", "number", "end"},
		"end":    []string{"end", "end", "end", "end"},
	}
	max, min, sign, ret, state := 2147483647, -2147483648, 1, 0, "start"
